...
import { ErrorMessage, FeedeeHash, PingMessage, PriceMessage, SocketMessage, SocketMessageData, SubMessage } from './types';
...
export const setupMessageHandler = (connection: connection, feedees: PriceFeedee[]) => {
	...
	connection.on('message', (data) => {
		...
		if (parsedMessage.hasOwnProperty('ch')) {
			...
			if (!handler) {
				return log('Spot websocket error: no handler provided for channel', priceMessage.ch);
			}
